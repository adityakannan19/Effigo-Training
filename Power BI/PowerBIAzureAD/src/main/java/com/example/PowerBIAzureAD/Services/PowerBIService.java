package com.example.PowerBIAzureAD.Services;
...
import com.example.PowerBIAzureAD.Config.Config;
import com.example.PowerBIAzureAD.Models.EmbedConfig;
import com.example.PowerBIAzureAD.Models.EmbedToken;
import com.example.PowerBIAzureAD.Models.ReportConfig;
...
public class PowerBIService {
	 ...
	 * @throws IOException 
	 */
	public static EmbedConfig getEmbedConfig(String accessToken, String workspaceId, String reportId,
			String... additionalDatasetIds) throws IOException {
		if (reportId == null || reportId.isEmpty()) {
			System.out.println("reportid empty?");
			throw new RuntimeException("Empty Report Id");
		}
		if (workspaceId == null || workspaceId.isEmpty()) {
			System.out.println("workspace id empty");
		...
		System.out.println(reportEmbedConfig);
	 ...
	 */
	public static EmbedConfig getEmbedConfig(String accessToken, String workspaceId, List<String> reportIds)
	 ...
	 */
	public static EmbedConfig getEmbedConfig(String accessToken, String workspaceId, List<String> reportIds,
			List<String> additionalDatasetIds) throws JsonMappingException, JsonProcessingException {
	 ...
	 * @throws JsonMappingException 
	 */
	//	public static EmbedToken getEmbedToken(String accessToken, String reportId, List<String> datasetIds,
	//			String... targetWorkspaceIds) throws JsonMappingException, JsonProcessingException {
	//		System.out.println("in getEmbed");
	...
	//		return embedToken;
	public static EmbedToken getEmbedToken(String accessToken, String reportId, List<String> datasetIds,
			String... targetWorkspaceIds) throws IOException {
		System.out.println("in getEmbed");
		final String uri = "https://api.powerbi.com/v1.0/myorg/GenerateToken";
		...
		requestBody.put("datasets", jsonDatasets);
		requestBody.put("reports", new JSONArray().put(new JSONObject().put("id", reportId)));
		JSONArray jsonWorkspaces = new JSONArray();
		for (String targetWorkspaceId : targetWorkspaceIds) {
			jsonWorkspaces.put(new JSONObject().put("id", targetWorkspaceId));
		}
		...
		try {
			...
			if (response.getStatusCode() == HttpStatus.UNAUTHORIZED) {
				throw new RuntimeException("Unauthorized: Access token may be expired or invalid");
			}
			...
			if (Config.DEBUG) {
				// Log embed token and request ID
				HttpHeaders responseHeader = response.getHeaders();
				List<String> reqIdList = responseHeader.get("RequestId");
				logger.info("Retrieved Embed token\nEmbed Token Id: {}", embedToken.tokenId);
				if (reqIdList != null && !reqIdList.isEmpty()) {
	 ...
	 * @throws JsonMappingException 
	 */
	public static EmbedToken getEmbedToken(String accessToken, List<String> reportIds, List<String> datasetIds,
			String... targetWorkspaceIds) throws JsonMappingException, JsonProcessingException {
